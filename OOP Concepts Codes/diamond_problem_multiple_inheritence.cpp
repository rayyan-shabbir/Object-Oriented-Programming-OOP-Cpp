...
//Sequence Matters,,, 1st-> B , then 2nd-> C class's features comes in class D
class D : public B, public C    //D inherits all the features of B and C except Constructors, Destructor, Assignment-operator and V-Table
{
	int dD;
	public:
		D() {
...
int main() {
	...
	od.test();  //--->> ERROR!!! Bcz object D (od) has two paths to go to test function of A. Thus, 'od' became ambiguous.

	od.display();  //--->> ERROR!!! Bcz object D (od) has two paths to go to display function of A. Thus, 'od' became ambiguous.
	return 0;
}