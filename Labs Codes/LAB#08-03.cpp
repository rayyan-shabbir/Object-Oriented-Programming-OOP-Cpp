...
public:
    ...
    Array<t> getSubArray(int start_index, int end_index);
...
template<class t>
void Array<t>::setElement(t k, int i)
{
    if (i >= 0 && i < size)
        data[i] = k;
    else
...
template<class t>
int Array<t>::countElement(t key)
{
    int count = 0;
    for (int i = 0; i < size; i++)
    {
...
template<class t>
Array<t> Array<t>::getSubArray(int start_index, int end_index)
{
    int count = 0;
